...
public abstract class BaseStreamControllerTests : BaseControllerTests
{
    ...
    public async Task StreamFile_WhenFileExists_SaveToTempStorage_ReturnSuccess()
    {
        ...
        // Assert
        streamFileResult.IsSuccess.Should().BeTrue();
        streamFileResult.Should().NotBeNull();

        await using var stream = streamFileResult.Value;
        await using var newLocalFile = await LocalFile.FromStreamAsync(stream, Path.GetTempPath(), Guid.NewGuid().ToString("N") + extension);

        var streamedFileCRC = Crc32Helper.CalculateFileCRC(newLocalFile.FilePath);
        streamedFileCRC.Should().Be(fileCRC);
    }
    ...
    public async Task StreamFile_WhenFileDoNotExist_ReturnFail()
    {
        ...
        // Act
        var streamFileResult = await storageClient.GetFileStream(Guid.NewGuid().ToString(), _streamEndpoint);

        // Assert
        streamFileResult.IsFailed.Should().BeTrue();
        streamFileResult.GetError().Value.ErrorCode.Should().Be(HttpStatusCode.InternalServerError.ToString());
    }
}