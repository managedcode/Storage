...
public interface IAzureDataLakeStorage : IStorage<DataLakeFileSystemClient, AzureDataLakeStorageOptions>
{
    ...
    Task<Result<Stream>> OpenWriteStreamAsync(OpenWriteStreamOptions options, CancellationToken cancellationToken = default);

    Task<Result<Stream>> OpenReadStreamAsync(OpenReadStreamOptions options, CancellationToken cancellationToken = default);
}