Project: ManagedCode.Storage.OneDrive
...
- OneDrive provider package that adapts Microsoft Graph-backed file storage to `IStorage`.
- Exists to isolate Graph client wiring and OneDrive auth concerns from the shared storage abstractions.

## Entry Points

- `OneDriveStorage.cs`
- `OneDriveStorageProvider.cs`
...
- Protected or high-risk areas: Graph client injection, delegated or app-only auth boundaries, and DI registration symmetry

## Project Commands

- `build`: `dotnet build ManagedCode.Storage.OneDrive.csproj`
...
- Preserve `GraphServiceClient` and auth responsibilities at the provider edge; do not leak Graph-specific types into Core or integrations.
