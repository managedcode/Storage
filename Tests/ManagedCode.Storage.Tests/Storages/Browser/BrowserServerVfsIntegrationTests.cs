...
{
    [Fact]
    public async Task BrowserStorage_ServerHost_VfsTextFlow_ShouldPersistMoveListAndDelete()
    {
        await using var context = await fixture.CreateContextAsync();
        var page = await context.NewPageAsync();
        var directory = $"server-browser-vfs-{Guid.NewGuid():N}";
    ...
    [Fact]
    public async Task BrowserStorage_ServerHost_VfsLargeFlow_ShouldPersistAcrossPages()
    {
        const int payloadSizeMiB = 16;

        await using var context = await fixture.CreateContextAsync();
        var firstPage = await context.NewPageAsync();
        var directory = $"server-browser-vfs-large-{Guid.NewGuid():N}";
    ...
    public async Task BrowserStorage_ServerHost_VfsConcurrentTabs_ShouldPersistAllFiles()
    {
        ...
        var directory = $"server-browser-vfs-concurrent-{Guid.NewGuid():N}";
