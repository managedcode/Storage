...
- **System:** ManagedCode.Storage is a .NET 10 storage platform with shared abstractions, provider packages, a virtual file system, ASP.NET delivery layers, client SDKs, Orleans grain persistence, and end-to-end integration tests.
- **Where is the code:** core contracts in `ManagedCode.Storage.Core/`, providers in `Storages/`, transport and integration packages in `Integraions/`, VFS in `ManagedCode.Storage.VirtualFileSystem/`, and end-to-end coverage in `Tests/ManagedCode.Storage.Tests/`.
...
- Allowed dependencies: provider packages may depend on `ManagedCode.Storage.Core`; integrations and VFS may depend on Core plus published provider abstractions; tests may depend on any package plus `ManagedCode.Storage.TestFakes`.
